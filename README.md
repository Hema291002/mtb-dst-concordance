...
**Promoter regions must be included.** The second commonest isoniazid resistance mutation lies 15 bases upstream of *fabG1*, outside the gene body. Target regions here extend 200 bp upstream, strand-aware. Restricting to coding sequence would have missed it in 3 isolates. The WHO catalogue names this same nucleotide `inhA_c.-777C>T`, relative to a different gene, which is why matching was done on coordinates rather than names ([`docs/findings/notation_mismatch.md`](docs/findings/notation_mismatch.md)).
